...
| `gm_campaigns.completed_reason` | scheduler `mark_campaign_completed`(既有唯一写方) | 透传/展示读方(C-003 §2 枚举);gm-e2e 子串白名单 | 新增取值 `SEARCH_EXHAUSTED`(列自由文本,无形状变化) |
...
**GREEN(实现子 agent)**:按 §2.2-a 真值表实现;前缀匹配用 `terminal_reason.as_deref().map(|r| r.trim_start().starts_with("NO_MORE_POSSIBLE_DATA")).unwrap_or(false)`。命令(scheduler 仓内):`cargo test schedule_evaluator`。
...
**GREEN(实现子 agent)**:实现 §2.2-c;MarkCompleted 分支把硬编码 `"ONCE_EXECUTED"` 替换为 `apply_once_completion(&eval_once_completion(task), ...)` 的返回值(`last_task` 在作用域内,completed 分支必有 `Some(task)`);WARN 含 campaign_id/task_id/process_count/max_count/terminal_reason。并落注释:
...
git diff main...HEAD --stat -- migrations/   # 期望:零命中(本仓亦不得有)
...
4. 每任务 RED→GREEN 证据齐备;「允许先绿」项(T1.11/T1.12、T3)各有 AG-006 证明或书面理由;T4 的 CI 实跑证据(或 PR 待开时的本地等价输出 + 跟进项)。
