...
| R-008 | 翻页中途页失败语义:本 task 已落库 >0 条 → `COMPLETED_WITH_PARTIAL_ERRORS`;零进展 → task `failed`(保留 eval_once 对 failed 的一次重派(每 tick 一次、跨 tick 无上限,DR-22),schedule_evaluator.rs:119-127) | derived | DECIDED | agent-rs/pagination-core | 失败注入 mock 测试两条(有进展/零进展);RED 证据 = 现状无此分支 | derived | B4(已扫已付成本);01 §5.4 |
