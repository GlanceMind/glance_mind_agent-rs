...
impl OpenAiAdapter {
    ...
    fn build_system_prompt(&self, context: &AnalysisContext) -> String {
        let target_audience = context.target_audience.as_deref().unwrap_or("General audience");
        let product_prompt = context.product_prompt.as_deref().unwrap_or("Products/Services");
        let dm_strategy = context.dm_strategy.as_deref().unwrap_or("Send personalized DM.");
        let reply_strategy = context.reply_strategy.as_deref().unwrap_or("Be helpful and friendly.");
        let reply_post_strategy = context.reply_post_strategy.as_deref().unwrap_or("Create engaging comment.");
...
"#)
    ...
    fn build_user_prompt(
        ...
        // Truncate video description if too long
        let desc = if content.description.len() > MAX_VIDEO_DESC_LENGTH {
            format!("{}...", &content.description[..MAX_VIDEO_DESC_LENGTH])
        } else {
        ...
        for comment in comments {
            ...
            let content_text = if comment.text.len() > MAX_COMMENT_LENGTH {
                format!("{}...", &comment.text[..MAX_COMMENT_LENGTH])
            } else {
                comment.text.clone()
            };
    ...
    fn parse_response(&self, response: &str, _comments: &[Comment]) -> Vec<ReplySuggestion> {
        ...
        match serde_json::from_str::<AgentOutput>(json_str) {
            Ok(parsed) => {
                parsed.suggestions.into_iter().map(|s| {
                    ReplySuggestion {
    ...
    /// Make the API call (matching Python glance_mind_agent)
    /// 
    ...
    async fn call_api(&self, system_prompt: &str, user_prompt: &str) -> AiResult<String> {
...
impl AiAnalyzer for OpenAiAdapter {
    async fn analyze_comment(
        ...
        let suggestions = self.analyze_batch(&[comment.clone()], content, context).await?;
        suggestions.into_iter().next().ok_or_else(|| {
            AiError::ServiceError("No suggestion generated".into())
        })
    }
...
mod tests {
    ...
    fn test_parse_intent() {
        ...
        assert_eq!(parse_intent("purchase_intent"), Some(CommentIntent::PurchaseIntent));
    ...
    fn test_parse_response() {
        ...
        assert_eq!(suggestions[0].reply_text, Some("Check your DMs!".to_string()));
        assert_eq!(suggestions[0].dm_text, Some("Hey John, I have something for you!".to_string()));
        assert_eq!(suggestions[0].post_reply_text, None);
        
        assert_eq!(suggestions[1].comment_id, "456");
        assert_eq!(suggestions[1].post_reply_text, Some("This is so relatable!".to_string()));
    }
