...
impl TikHubError {
    ...
    pub fn from_status(status: u16, message: impl Into<String>) -> Self {
        ...
        match status {
            ...
            429 => TikHubError::RateLimited { retry_after_secs: Some(60) },
...
mod tests {
    ...
    #[test]
    fn test_retry_delay() {
        let err = TikHubError::RateLimited { retry_after_secs: Some(30) };
        assert_eq!(err.retry_delay_ms(), Some(30000));
        
        let err = TikHubError::RateLimited { retry_after_secs: None };
        assert_eq!(err.retry_delay_ms(), Some(60000)); // Default 60s
        
        let err = TikHubError::ServerError { status: 500, message: "".to_string() };
        assert_eq!(err.retry_delay_ms(), Some(2000));
    }
    
    #[test]
    fn test_max_retries() {
        assert_eq!(TikHubError::RateLimited { retry_after_secs: None }.max_retries(), 3);
        assert_eq!(TikHubError::BadRequest { message: "".to_string() }.max_retries(), 1);
        assert_eq!(TikHubError::Unauthorized { message: "".to_string() }.max_retries(), 0);
    }
