...
async fn main() -> anyhow::Result<()> {
    ...
    if cli.full {
        ...
        info!("🔍 Generating search fixture for keyword '{}'...\n", keyword);
        let path = generator.generate_search_fixture(&keyword, &cli.region, cli.count).await?;
        info!("\n✅ Saved to: {}", path.display());
    } else if let Some(video_id) = cli.video_id {
        info!("💬 Generating comments fixture for video {}...\n", video_id);
        let path = generator.generate_comments_fixture(&video_id, cli.count).await?;
        info!("\n✅ Saved to: {}", path.display());
    } else if let Some(username) = cli.username {
        info!("👤 Generating user videos fixture for @{}...\n", username);
        let path = generator.generate_user_videos_fixture(&username, cli.count).await?;
        info!("\n✅ Saved to: {}", path.display());
    } else {
